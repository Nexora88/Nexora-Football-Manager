(() => {
  ...
  function setTribunesColor(homeColor = '#b7ff3c', awayColor = '#ffffff') {
    ...
    if (!host) return;
    host.style.setProperty('--home-color', homeColor);
    host.style.setProperty('--away-color', awayColor);
  ...
  function generateAttendance({ reputation = gameState.reputation || 0, ticketPrice = 18, importance = 'normal' } = {}) {
    ...
    stadium.ticketPrice = ticketPrice;
    stadium.matchImportance = importance;
    stadium.attendanceRate = rate;
    stadium.attendance = Math.round(stadium.capacity * rate);
  ...
  function renderTribunes(attendanceRate) {
    const host = document.getElementById('stadiumTribunes');
    if (!host) return;
    const layers = [...host.querySelectorAll('.tribune-layer')];
  ...
  function triggerGoalCelebration() {
    ...
    if (!host) return;
    host.classList.remove('goal-celebration');
    void host.offsetWidth;
    host.classList.add('goal-celebration');
  ...
  function refreshStadium() {
    ...
    const attendanceData = generateAttendance({
      ...
      importance: stadium.matchImportance || 'normal'
    });
    const attendance = attendanceData.attendance;
    const occupancy = Math.round(attendanceData.rate * 100);
    ...
    setTribunesColor('#b7ff3c', '#d7dde0');
    renderTribunes(occupancy / 100);
    ...
    const goal = document.getElementById('goalTest');
    if (goal) goal.addEventListener('click', triggerGoalCelebration);
  ...
  function startConstruction(targetLevel) {
    ...
    stadium.construction = { targetLevel, endDate: addDays(gameState.date, target.days) };
