(()=>{
  ...
  const ensure=s=>{
    s.gameState.finance ||= {wageBill:0,wageBudget:Math.round((s.gameState.money||0)*.025),seasonWagePaid:0};
  ...
  function dailyNews(s){
    const m=s.gameState.media;const d=dateObj(s.gameState.date);const day=d.getDay();
    const stamp=s.gameState.date;
    if(m.lastDailyNews===stamp)return;
    const club=s.gameState.club?.name||'Kulübün';
    const pool=[
  ...
  function developYouth(s){
    ...
    const training=s.gameState.facilities?.training?.level||0;
    const academy=a.level||0;
    const today=dateObj(s.gameState.date);
    a.signed.forEach(p=>{
      const last=dateObj(p.lastDevelopment||s.gameState.date);const weeks=Math.floor((today-last)/604800000);if(weeks<=0)return;
      for(let i=0;i<weeks;i++){
        const ceiling=Number(p.potential)||70;
        const gain=(0.15+academy*.10+training*.08)*(Math.random()>.18?1:0);
        p.rating=Math.min(ceiling,Math.round((Number(p.rating)||50)+gain));
        p.value=Math.max(Number(p.value)||50000,Math.round((p.rating||50)*(p.rating||50)*900));
        p.morale=Math.max(45,Math.min(100,(p.morale||75)+(Math.random()*6-2)));
      }
  ...
  function processDay(s){
    ...
    dailyNews(s);
    s.gameState.finance.wageBill=calcWages(s);
    s.gameState.finance.lastProcessed=s.gameState.date;
    save(s);
  }
  function syncUI(s){
    if(window.syncDashboard)window.syncDashboard();
    window.NEXORA_FACILITIES?.render?.();
    window.NEXORA_MEDIA?.render?.();
    window.NEXORA_TRANSFER?.render?.();
    window.dispatchEvent(new CustomEvent('nexora:day-advanced',{detail:{date:s.gameState.date}}));
  }
  function advance(days=1){
    const s=read();if(!s?.gameState?.club)return;
    for(let i=0;i<days;i++){s.gameState.date=dateAdd(s.gameState.date,1);processDay(s);}
    save(s);syncUI(s);return s.gameState.date;
  }
  function renderFinance(s){
    const dash=document.getElementById('dashboard');if(!dash||!s?.gameState?.club)return;
    let host=document.getElementById('gameFinance');
  ...
  window.addEventListener('nexora:transfer-updated',()=>{const s=read();if(s){ensure(s);save(s);renderFinance(s);}});
})();