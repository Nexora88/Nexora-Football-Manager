...
const continueCareerBtn=document.getElementById('continueCareerBtn');
...
/* CORE GAME STATE â€” the single source of truth for the career. */
...
function formatMoney(value){return `â‚¬${(value/1000000).toFixed(1)}M`}
...
function renderClubs(){
 clubGrid.innerHTML=clubs.map((club,i)=>`<button class="club-choice" data-index="${i}"><b>${club.code}</b><strong>${club.name}</strong><span>${club.city} Â· ${club.style} Â· ${club.difficulty}</span><small>â‚¬${(club.budget/1000000).toFixed(1)}M BUDGET Â· ${club.stadiumCapacity.toLocaleString('en-US')} CAP.</small></button>`).join('');
...
function syncDashboard(){
 document.getElementById('dashClub').textContent=gameState.club?.name||'â€”';
 document.getElementById('dashCity').textContent=gameState.club?.city||'â€”';
 document.getElementById('dashManager').textContent=gameState.managerName||'â€”';
 document.getElementById('dashStyle').textContent=gameState.managerStyle||'â€”';
...
function beginCareer(club){try{
 ...
 syncDashboard();document.getElementById('welcomeTitle').textContent=`Congratulations, ${managerName}.`;document.getElementById('welcomeSub').textContent=`${canonical.city} Â· ${canonical.name} Â· Your 2026/27 journey starts now.`;
 closeModal();hero.hidden=true;dashboard.hidden=false;saveState();window.scrollTo({top:0,behavior:'smooth'});window.NEXORA_I18N?.refresh?.();window.NEXORA_HOME_I18N?.refresh?.();window.dispatchEvent(new CustomEvent('nexora:career-ready'));}catch(error){console.error('[NEXORA] beginCareer failed',error);hero.hidden=true;dashboard.hidden=false;}
}
...
function loadState(silent=false){try{const saved=JSON.parse(localStorage.getItem('nexoraCareer')||'null');if(!saved?.gameState?.club)return;Object.assign(gameState,saved.gameState);career=saved.career||{};squad=saved.squad||[];startingXI=saved.startingXI||[];formation=saved.formation||'4-3-3';selectedClub=gameState.club;selectedStyle=gameState.managerStyle||'Tactical Genius';if(!squad.length)squad=makeSquad(gameState.club);syncDashboard();document.getElementById('welcomeTitle').textContent=`Welcome back, ${gameState.managerName}.`;document.getElementById('welcomeSub').textContent=`${gameState.club.city} Â· ${gameState.club.name} Â· ${formatDate(gameState.date)}`;hero.hidden=true;dashboard.hidden=false;window.NEXORA_I18N?.refresh?.();window.NEXORA_HOME_I18N?.refresh?.();if(!silent)window.dispatchEvent(new CustomEvent('nexora:career-ready'))}catch(error){console.warn('[NEXORA] Career restore failed',error)}}
window.loadState=loadState;
function hasSavedCareer(){try{return !!JSON.parse(localStorage.getItem('nexoraCareer')||'null')?.gameState?.club}catch(error){console.warn('[NEXORA] saved career check failed',error);return false}}
function refreshContinueCareer(){if(continueCareerBtn)continueCareerBtn.hidden=!hasSavedCareer();window.NEXORA_I18N?.refresh?.();window.NEXORA_HOME_I18N?.refresh?.()}
...
function openSquad(){try{if(!career.club&&!gameState.club){console.warn('[NEXORA] openSquad: no club');return}const area=document.getElementById('gameArea')||document.createElement('section');area.id='gameArea';area.className='game-area';dashboard.appendChild(area);renderSquad();area.scrollIntoView({behavior:'smooth'})}catch(error){console.error('[NEXORA] openSquad failed',error)}}
function renderSquad(){try{if(!career.club)career.club=gameState.club;if(!career.club){console.warn('[NEXORA] renderSquad: no club');return}
 const area=document.getElementById('gameArea'),m=teamMetrics();
 area.innerHTML=`<div class="game-head"><div><span class="eyebrow">SQUAD / ${career.club.code}</span><h2>FIRST TEAM</h2><p>Player attributes now drive the simulation: pace, passing, shooting, defending, dribbling, physicality and vision.</p></div><div class="game-actions"><button class="secondary" id="backDash">DASHBOARD</button><button class="primary" id="tacticsBtn">TACTICS â†’</button></div></div><div class="squad-layout"><div class="player-list"><div class="list-head"><span>PLAYER</span><span>POS</span><span>OVR</span><span>FIT</span></div>${squad.map(p=>`<button class="player-row ${startingXI.includes(p.id)?'starter':''}" data-player="${p.id}"><strong>${p.name}</strong><span>${p.pos}</span><b>${p.rating}</b><span>${p.fitness}%</span></button>`).join('')}</div><div class="squad-summary"><span class="eyebrow">DATA PROFILE</span><h3>${startingXI.length}/11 STARTERS</h3><p>Each player has a full simulation profile. Select your XI to change team performance.</p><div class="mini-stat"><span>OVERALL</span><b>${m.rating}</b></div><div class="mini-stat"><span>PACE / PASS</span><b>${m.pace} / ${m.passing}</b></div><div class="mini-stat"><span>SHOT / DRIBBLE</span><b>${m.shooting} / ${m.dribbling}</b></div><div class="mini-stat"><span>DEF / PHYS</span><b>${m.defending} / ${m.physical}</b></div><div class="mini-stat"><span>VISION</span><b>${m.vision}</b></div><button class="primary full" id="matchFromSquad" ${startingXI.length!==11?'disabled':''}>PREPARE MATCH â†’</button></div></div>`;
 document.querySelectorAll('.player-row').forEach(row=>row.addEventListener('click',()=>toggleStarter(row.dataset.player)));document.getElementById('tacticsBtn').addEventListener('click',renderTactics);document.getElementById('matchFromSquad').addEventListener('click',startMatch);document.getElementById('backDash').addEventListener('click',()=>window.scrollTo({top:0,behavior:'smooth'}));window.NEXORA_I18N?.refresh?.();window.NEXORA_HOME_I18N?.refresh?.()}catch(error){console.error('[NEXORA] renderSquad failed',error)}}
function toggleStarter(id){id=String(id);if(startingXI.includes(id))startingXI=startingXI.filter(x=>x!==id);else if(startingXI.length<11)startingXI=[...startingXI,id];else return;persist();renderSquad()}

function renderTactics(){try{if(!career.club)career.club=gameState.club;if(!career.club){console.warn('[NEXORA] renderTactics: no club');return}
 const area=document.getElementById('gameArea'),m=teamMetrics(),forms=['4-3-3','4-2-3-1','4-4-2'];
 area.innerHTML=`<div class="game-head"><div><span class="eyebrow">TACTICAL BOARD / 2.5D READY</span><h2>YOUR GAME PLAN</h2><p>The pitch uses a 100 Ã— 60 logical grid. Player attributes will drive movement and actions.</p></div><button class="secondary" id="squadBack">â† SQUAD</button></div><div class="tactics-layout"><div class="pitch pitch-25d"><div class="pitch-line"></div><div class="pitch-box top"></div><div class="pitch-box bottom"></div><div class="center-circle"></div><div class="pitch-label">${career.club.code}</div></div><div class="tactic-panel"><span class="eyebrow">FORMATION</span><div class="formation-grid">${forms.map(f=>`<button class="formation ${formation===f?'active':''}" data-form="${f}">${f}</button>`).join('')}</div><div class="mini-stat"><span>TEAM OVERALL</span><b>${m.rating}</b></div><div class="mini-stat"><span>PASSING / VISION</span><b>${m.passing} / ${m.vision}</b></div><div class="mini-stat"><span>ATTACK / DEFENCE</span><b>${Math.round((m.shooting+m.dribbling+m.pace)/3)} / ${m.defending}</b></div><div class="mini-stat"><span>MANAGER</span><b>${gameState.managerName}</b></div><div class="mini-stat"><span>OPPONENT</span><b>${opponent.name}</b></div><button class="primary full" id="startMatchTactic" ${startingXI.length!==11?'disabled':''}>START MATCH â†’</button></div></div>`;
 document.querySelectorAll('.formation').forEach(b=>b.addEventListener('click',()=>{formation=b.dataset.form;gameState.formation=formation;persist();renderTactics()}));document.getElementById('squadBack').addEventListener('click',renderSquad);document.getElementById('startMatchTactic').addEventListener('click',startMatch);window.NEXORA_I18N?.refresh?.();window.NEXORA_HOME_I18N?.refresh?.()}catch(error){console.error('[NEXORA] renderTactics failed',error)}}
...
function startMatch(){
 ...
 const goalHome=()=>{homeGoals++;const scorer=squad.find(p=>p.id===startingXI[Math.floor(Math.random()*startingXI.length)]);addEvent(`GOAL! ${scorer.name} scores. SHOT ${scorer.shooting} Â· PACE ${scorer.pace}.`);document.getElementById('homeScore').textContent=homeGoals;if(window.triggerGoalCelebration)window.triggerGoalCelebration()};
...
function finishMatch(home,away){clearInterval(matchTimer);career.board=Math.max(0,Math.min(100,career.board+(home>away?5:home===away?1:-5)));career.reputation=Math.max(0,career.reputation+(home>away?2:home===away?1:-2));gameState.boardConfidence=career.board;gameState.reputation=career.reputation;persist();syncDashboard();const log=document.getElementById('eventLog');if(log)log.insertAdjacentHTML('afterbegin',`<div class="final-event">FULL TIME Â· ${home} â€” ${away}</div><button class="primary full" id="returnSquad">RETURN TO SQUAD</button>`);const btn=document.getElementById('returnSquad');if(btn)btn.addEventListener('click',renderSquad)}

startBtn.addEventListener('click',()=>{try{if(hasSavedCareer()&&!confirm('Mevcut kariyerinizin üzerine yeni bir kariyer başlatmak istediğinize emin misiniz?'))return;localStorage.removeItem('nexoraCareer');localStorage.removeItem('nexoraGameState');window.NEXORA_SKIP_INTRO=false;renderClubs();openModal();window.NEXORA_I18N?.refresh?.();window.NEXORA_HOME_I18N?.refresh?.()}catch(error){console.error('[NEXORA] start career failed',error)}});demoBtn.addEventListener('click',()=>{renderClubs();openModal()});closeBtn.addEventListener('click',closeModal);choices.forEach(choice=>choice.addEventListener('click',()=>{choices.forEach(c=>c.classList.remove('selected'));choice.classList.add('selected');selectedStyle=choice.dataset.style||choice.querySelector('strong').textContent}));continueBtn.addEventListener('click',()=>{if(!selectedClub){selectedClub=clubs[0];document.querySelector('.club-choice').classList.add('selected')}beginCareer(selectedClub)});createClubBtn.addEventListener('click',()=>{careerStep.hidden=true;createStep.hidden=false});saveClubBtn.addEventListener('click',()=>{const name=document.getElementById('clubName').value.trim(),city=document.getElementById('clubCity').value.trim();if(!name||!city){alert('KulÃ¼p adÄ± ve ÅŸehir girin.');return}selectedClub={id:`custom-${Date.now()}`,name,city,code:(name.replace(/[^a-zA-Z0-9ÄŸÃ¼ÅŸÃ¶Ã§Ä±Ä°ÄÃœÅÃ–Ã‡]/g,'').slice(0,3)||'NEX').toUpperCase(),style:'Balanced',league:'Nexora Super League',difficulty:'CUSTOM',budget:4000000,reputation:40,stadiumCapacity:5000,colors:['#b7ff3c','#ffffff']};createStep.hidden=true;careerStep.hidden=false;beginCareer(selectedClub)});squadBtn.addEventListener('click',openSquad);
continueCareerBtn?.addEventListener('click',()=>{try{window.NEXORA_SKIP_INTRO=true;loadState(true);if(!gameState.club){refreshContinueCareer();return}hero.hidden=true;dashboard.hidden=false;document.getElementById('nexoraAvatarModal')?.remove();document.querySelector('.airport-vip')?.remove();window.NEXORA_I18N?.refresh?.();window.NEXORA_HOME_I18N?.refresh?.();refreshContinueCareer()}catch(error){console.error('[NEXORA] continue career failed',error)}});backToStart.addEventListener('click',()=>{try{if(hasSavedCareer()&&!confirm('Mevcut kariyeri silip yeni bir kariyer başlatmak istediğinize emin misiniz?'))return;clearInterval(matchTimer);localStorage.removeItem('nexoraCareer');localStorage.removeItem('nexoraGameState');window.NEXORA_SKIP_INTRO=false;dashboard.hidden=true;hero.hidden=false;selectedClub=null;selectedStyle='Tactical Genius';career={};squad=[];startingXI=[];const area=document.getElementById('gameArea');if(area)area.remove();window.scrollTo({top:0,behavior:'smooth'});refreshContinueCareer();window.NEXORA_HOME_I18N?.refresh?.()}catch(error){console.error('[NEXORA] new career failed',error)}});modal.addEventListener('click',e=>{if(e.target===modal)closeModal()});document.addEventListener('keydown',e=>{if(e.key==='Escape')closeModal()});refreshContinueCareer();
